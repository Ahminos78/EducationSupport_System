...
-- ======================== 6. 作业数据 ========================
INSERT INTO tb_assignment (id, course_id, teacher_id, title, description, full_score, deadline, status, deleted) VALUES
  (1, 9101, 1000, '第一次作业：登录接口联调', '完成登录接口调用，记录请求、响应和异常处理。', 100, '2026-08-20 23:59:59', 1, 0),
  (2, 9101, 1000, '第二次作业：课程模块 CRUD', '完成课程列表、新增、编辑和状态修改接口测试。', 100, '2026-08-27 23:59:59', 0, 0),
  (3, 9102, 1000, 'MyBatis Plus 实体映射练习', '整理实体类注解、Mapper 继承和逻辑删除说明。', 100, '2026-08-25 23:59:59', 1, 0),
  (4, 9103, 1000, 'MyBatis Plus 高级查询', '练习条件构造器和分页查询。', 100, '2026-08-30 23:59:59', 0, 0),
  (5, 9104, 1000, '微服务架构设计', '画出系统架构图，说明各服务职责。', 100, '2026-09-05 23:59:59', 1, 0),
  (6, 9105, 1000, '排序算法实现', '实现快速排序和归并排序。', 100, '2026-08-22 23:59:59', 0, 0),
  (7, 9201, 1001, 'Pandas 基础练习', '完成 DataFrame 基本操作。', 100, '2026-08-20 23:59:59', 1, 0),
  (8, 9202, 1001, 'Vue3 组件拆分', '完成登录页和基础布局组件拆分。', 100, '2026-08-18 23:59:59', 2, 0),
  (9, 9203, 1001, '机器学习入门报告', '撰写 KNN 算法学习报告。', 100, '2026-09-01 23:59:59', 0, 0)
...
-- ======================== 9. 讨论数据 ========================
INSERT INTO tb_discussion (course_id, parent_id, author_id, title, content, status, deleted) VALUES
  (9101, NULL, 201, '登录接口返回 401 怎么排查？', '我在调用 /api/users/me 时遇到了 401，想确认是不是 Token 传递问题。', 1, 0),
  (9101, 1, 1000, NULL, '先确认请求头是否是 Authorization: Bearer <token>，再看网关是否透传。', 1, 0),
  (9101, 1, 202, NULL, '我之前是少写了 Bearer，补上之后就好了。', 1, 0),
  (9102, NULL, 201, 'BaseMapper 和自定义 SQL 应该怎么取舍？', '简单 CRUD 是否都推荐改成 MyBatis Plus 内置方法？', 1, 0),
  (9102, 4, 1000, NULL, '复杂联表查询保留自定义 SQL，简单插入和按 ID 查询可以优先用 BaseMapper。', 1, 0),
  (9105, NULL, 201, '快速排序时间复杂度讨论', '为什么最坏情况是 O(n²)？如何避免？', 1, 0),
  (9201, NULL, 205, 'Pandas 和 SQL 的异同', '两者在数据处理上各自有什么优势？', 1, 0),
  (9201, 7, 1001, NULL, 'Pandas 适合交互式探索，SQL 适合批量数据处理。', 1, 0)
ON DUPLICATE KEY UPDATE title=VALUES(title);
